...
import { SeeAndReportPage } from './components/SeeAndReportPage';
import { projectId } from './utils/supabase/info';

type PageType = 'home' | 'location-insights' | 'ai-assistant' | 'collector' | 'hub-manager' | 'hub-transaction' | 'register-collector' | 'view-collectors' | 'hub-transactions' | 'hub-settings' | 'data' | 'about' | 'contact' | 'login' | 'volunteer' | 'donations' | 'messaging' | 'blog' | 'see-report';

function AppContent() {
  const [currentPage, setCurrentPage] = useState<PageType>('home');
  const { user, isAuthenticated, loading } = useAuth();

  // Initialize phone mapping for demo account (one-time setup)
  useEffect(() => {
    const initializeDemoPhone = async () => {
      try {
        console.log('Initializing demo phone mappings...');
        const response = await fetch(`https://${projectId}.supabase.co/functions/v1/make-server-6c51ae02/setup-demo-phone`, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          console.log('✅ Demo phone mapping initialized successfully:', data);
        } else {
          console.log('❌ Demo phone mapping failed:', data);
        }
      } catch (error) {
        console.log('⚠️ Phone mapping initialization error (non-critical):', error);
      }
    };
  ...
  // Check if current page is a collector dashboard page
  const isCollectorDashboardPage = isAuthenticated && user?.role === 'collector' && 
    ['collector', 'location-insights', 'messaging'].includes(currentPage);

  return (
    <div className="min-h-screen bg-white">
      {/* Only show main navigation if not on hub manager or collector dashboard pages */}
      {!isHubManagerPage && !isCollectorDashboardPage && <Navigation currentPage={currentPage} onNavigate={handleNavigate} />}
      ...
      <main>
        ...
        {isHubManagerPage ? (
        ...
        ) : isCollectorDashboardPage ? (
          <CollectorLayout currentPage={currentPage} onNavigate={handleNavigate}>
            {currentPage === 'collector' && <CollectorDashboard onNavigate={handleNavigate} />}
          ...
          <>
            ...
            {currentPage === 'see-report' && <SeeAndReportPage onNavigate={handleNavigate} />}
      ...
      {/* Footer - Only show on public pages (not hub manager or collector dashboard) */}
      {!isHubManagerPage && !isCollectorDashboardPage && (
