...
// @desc    Get overall impact aggregation across all donations
// @route   GET /api/donations/impact/aggregate
// @access  Public
const getAggregateImpact = asyncHandler(async (req, res) => {
  const { timeRange = 'all', status = 'completed' } = req.query;
  
  const aggregateImpact = await Donation.getAggregateImpact(timeRange, status);
  
  res.json({
    success: true,
    data: { impact: aggregateImpact }
  });
});
