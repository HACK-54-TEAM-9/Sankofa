...
const { sendVerificationEmail, sendSMS: sendNotificationSMS } = require('../services/notificationService');
...
const register = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    const userData = {
      ...
      role: role || 'collector',
      status: process.env.NODE_ENV === 'development' ? 'active' : 'pending', // Auto-activate in dev
      is_email_verified: process.env.NODE_ENV === 'development' ? true : false,
      is_phone_verified: process.env.NODE_ENV === 'development' ? true : false,
    ...
    try {
      ...
      await sendNotificationSMS({
        to: user.phone,
        message: `Your Sankofa verification code is: ${verificationCode}`
      });
      logger.info('Verification SMS sent', { userId: user.id });
      // await sendSMS({
      //   to: user.phone,
